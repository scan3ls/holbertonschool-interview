...
avl_t *build_avl(int *array, size_t size)
{
	...
	for (i = 0; i < range; i++)
	{
		if ((int)size % 2)
			...
			array_left[i] = array[i - 1];
	}

	for (i = 0; i < (int)size / 2; i++)
	{
		if ((int)size % 2 == 0)
			array_right[i] = array[i + (size / 2)];
		else
			array_right[i] = array[i + 1 + (size / 2)];
	}

	root->n = middle;
	root->parent = NULL;
	root->left = build_avl(array_left, (size / 2));
	root->right = build_avl(array_right, (size / 2));
